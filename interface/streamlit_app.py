...
def main():
    ...
    # Camera input
    # image = st.camera_input("Take a picture of a Pokemon card!")
    ...
    if uploaded == True:
        ...
        st.markdown(
            f"""<h3 class='general-text'>Trading at <span class='market-price'>{market_price}</span> USD today!</h3>""",
            unsafe_allow_html=True,
        )
        # Printing the image

        # st.image(image_url)
        columns[1].image(image_url, caption="Uploaded Image.", use_column_width=True)

        # Ask if this is the correct card
        correct = st.sidebar.radio("Is this the correct card?", ["Yes", "No"])
